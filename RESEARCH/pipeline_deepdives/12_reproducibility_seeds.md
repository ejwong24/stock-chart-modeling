...
1. **Debugging.** When a backtest produces a suspicious Sharpe ratio or an outlier trade, we need to rerun the exact same scenario, drop into the offending step, and inspect intermediate state. A pipeline that produces different blotters on every run gives us nothing to diff against.
...
- LightGBM / XGBoost without `random_state` or `seed`
...
| `src/stock_chart/models.py` | `fit_lr_baseline` — LogisticRegression `random_state` | `seed` param | 42 |
| `src/stock_chart/models.py` | `fit_lgbm` — LightGBM `random_state` | `seed` param | 42 |
...
| `src/stock_chart/embed_dinov2.py` | DINOv2 inference | — | deterministic (eval mode, no dropout) |
...
## DINOv2 — deterministic, with a caveat
...
- `tests/test_pipeline_integration.py::test_full_pipeline_determinism_same_seed` — runs the full chain twice with the same seeds and asserts bit-identical blotter and equity DataFrames.
...
The reproducibility audit ran **17 hardening passes**. One of them — `test_full_pipeline_determinism_same_seed` — confirms that running the full chain end-to-end twice, with the same seeds and the same data snapshot, produces bit-identical blotter and equity outputs. That is the load-bearing guarantee.
